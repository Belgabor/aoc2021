...
function is_done(situation: Situation): boolean {
    return (
        situation.state['rA1']?.[0] === 'A'
        && situation.state['rA2']?.[0] === 'A'
        && situation.state['rB1']?.[0] === 'B'
        && situation.state['rB2']?.[0] === 'B'
        && situation.state['rC1']?.[0] === 'C'
        && situation.state['rC2']?.[0] === 'C'
        && situation.state['rD1']?.[0] === 'D'
        && situation.state['rD2']?.[0] === 'D'
    )
}

function perform_move(situation: Situation, move: Move): Maybe<Situation> {
...
function evaluate(situation: Situation, depth: number) {
    if (situation.energy >= best) {
        return
    }
    // console.log(depth, situation.energy, best)
    if (depth > max_depth) {
        return
    }

...
const all_beacons: Location[] = []
for (const scanner of Object.values(done_scanners)) {
    all_beacons.push(...resolveBeacons(scanner))
}

const dedupe = new Set(all_beacons.map(loc => `${loc[0]},${loc[1]},${loc[2]}`))
console.log(dedupe.size)
